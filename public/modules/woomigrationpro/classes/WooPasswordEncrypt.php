...
class WooPasswordEncrypt
{
    ...
    public function getRandomBytes($count)
    {
        ...
        if (Tools::strlen($output) < $count) {
            ...
            $output = Tools::substr($output, 0, $count);
        }
    ...
    public function hashPassword($password)
    {
        ...
        if (CRYPT_BLOWFISH == 1 && !$this->portable_hashes) {
            ...
            if (Tools::strlen($hash) == 60) {
                return $hash;
            }
        }

        if (CRYPT_EXT_DES == 1 && !$this->portable_hashes) {
            if (Tools::strlen($random) < 3) {
                $random = $this->getRandomBytes(3);
            }
            $hash =
                crypt($password, $this->genSaltExtended($random));
            if (Tools::strlen($hash) == 20) {
                return $hash;
            }
        }

        if (Tools::strlen($random) < 6) {
